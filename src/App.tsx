...
function App() {
  ...
  const saveProduct = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const name = String(form.get('name') ?? '').trim(); const sellingPrice = Number(form.get('sellingPrice')); const purchasePrice = Number(form.get('purchasePrice')); const stock = Number(form.get('stock')); const minimumStock = Number(form.get('minimumStock'))
  ...
  return <div className="app-shell">
    ...
    {(showProductForm || editingProduct) && <ProductModal key={editingProduct?.id ?? 'new'} state={state} product={editingProduct} onClose={() => { setShowProductForm(false); setEditingProduct(null) }} onSubmit={saveProduct} onScan={scanProductCode} />}
