...
import type { BackupFile, Category, Customer, Expense, PosState, Product, StockMovement, StoreSettings, Transaction } from './types'
...
const demoProducts: Product[] = [
  { id: 'prod-indomie', sku: 'MKN-001', barcode: '8992388101001', name: 'Indomie Goreng', categoryId: 'cat-food', purchasePrice: 2500, sellingPrice: 3500, stock: 48, minimumStock: 10, unit: 'pcs', isActive: true, createdAt: now(), updatedAt: now() },
  { id: 'prod-aqua', sku: 'MNM-001', barcode: '8992775101001', name: 'Aqua 600ml', categoryId: 'cat-drink', purchasePrice: 2500, sellingPrice: 4000, stock: 36, minimumStock: 12, unit: 'botol', isActive: true, createdAt: now(), updatedAt: now() },
  { id: 'prod-teh', sku: 'MNM-002', barcode: '8998866101001', name: 'Teh Botol Sosro', categoryId: 'cat-drink', purchasePrice: 3500, sellingPrice: 5000, stock: 22, minimumStock: 8, unit: 'botol', isActive: true, createdAt: now(), updatedAt: now() },
  { id: 'prod-kopi', sku: 'MKN-002', barcode: '8991002101001', name: 'Kopi Sachet', categoryId: 'cat-food', purchasePrice: 1200, sellingPrice: 2000, stock: 8, minimumStock: 10, unit: 'pcs', isActive: true, createdAt: now(), updatedAt: now() },
  { id: 'prod-roti', sku: 'MKN-003', barcode: '8999999101001', name: 'Roti Tawar', categoryId: 'cat-food', purchasePrice: 9000, sellingPrice: 12000, stock: 0, minimumStock: 4, unit: 'bungkus', isActive: true, createdAt: now(), updatedAt: now() },
]
...
export const createEmptyState = (): PosState => ({
  ...
  expenses: [],
  stockMovements: [],
  settings: { storeName: 'Toko Kita', storeLogo: '', storeAddress: 'Alamat toko belum diatur', storePhone: '', taxRate: 0, allowNegativeStock: false, theme: 'light', receiptFooter: 'Terima kasih sudah berbelanja', autoPrintReceipt: false },
})
...
async function getNativeDb() {
  ...
  try {
    ...
    `)
    await nativeDb.run('INSERT OR IGNORE INTO schema_migrations (version, applied_at) VALUES (?, ?)', [DB_VERSION, now()])
    return nativeDb
  } catch (error) {
...
export function createBackup(state: PosState): BackupFile {
  return { format: 'POS Backup', version: DB_VERSION, createdAt: now(), data: state }
}
...
export type { Category, Customer, Expense, PosState, Product, StockMovement, StoreSettings, Transaction }
